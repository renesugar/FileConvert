...
// @TODO(alex): remove the ifdef
 ...
 private:
  ...
  bool notnull;              // true if required; "NULL for column" exception thrown on import
...
inline std::string BooleanToString(bool b) {
  return (b ? "t" : "f");
}

inline std::string DecimalToString(int64_t value, int precision, int scale) {
  double v = (static_cast<double>(value)) / pow(10, scale);
  int len = std::snprintf(nullptr, 0, "%*.*f", precision, scale, v);
  std::vector<char> str(len + 1);
  std::snprintf(&str[0], str.size(), "%*.*f", precision, scale, v);
  str[sizeof(str)-1] = '\0';
...
inline std::string TimestampToString(time_t t) {
  std::tm tm_struct;
  gmtime_r(&t, &tm_struct);
  char buf[20];
...
inline std::string DateToString(time_t t) {
  std::tm tm_struct;
  gmtime_r(&t, &tm_struct);
  char buf[11];
...
inline std::string IntervalDayTimeToString(time_t t) {
  return std::to_string(t) + " ms (day-time interval)";
}

inline std::string IntervalYearMonthToString(time_t t) {
  return std::to_string(t) + " month(s) (year-month interval)";
}
